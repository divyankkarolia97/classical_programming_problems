/*
	Program creates a min segment tree of size n (input from user) and processes q queries of type
	1 l r 			queries the segment tree for minimum value in range [l,r] 
	2 l r update 	updates the segment tree in range [l,r] with value update 
	
	Segment Tree is a pretty efficient data structure for range query as it is created in O(n) and updation and querying takes place in O(logn).	
*/
